package cn.sanleny.study.springcloud.client.hystrix;

import cn.sanleny.study.springcloud.client.service.QueryServiceRemoteCall;
...
public class MovieCollapserCommand extends HystrixCollapser<List<Map<String,Object>>,Map<String,Object>,String> {
    ...
    public MovieCollapserCommand(QueryServiceRemoteCall queryServiceRemoteCall, String movieCode) {
        super(Setter.withCollapserKey(HystrixCollapserKey.Factory.asKey("MovieCollapserCommand"))
     ...
     */
    @Override
    protected HystrixCommand<List<Map<String, Object>>> createCommand(Collection<CollapsedRequest<Map<String, Object>, String>> collapsedRequests) {
        //取出这个时间窗口下的所有请求
        //将参数定义为一个集合
        List<String> movieIds = new ArrayList<>(collapsedRequests.size());
        movieIds.addAll(collapsedRequests.stream().map(CollapsedRequest::getArgument).collect(Collectors.toList()));
     ...
     */
    @Override
    protected void mapResponseToRequests(List<Map<String, Object>> batchResponse, Collection<CollapsedRequest<Map<String, Object>, String>> collapsedRequests) {
